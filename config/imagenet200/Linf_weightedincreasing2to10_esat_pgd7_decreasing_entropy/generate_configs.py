...
with open('./template.yaml', 'r') as f:
    template = f.read()
name_template = 'resnet50_A{nA}.PGDs2i7e8_B{nB}.none'
...
for i in range(1,N):
    ...
    if nA == 1:
        f_run.write(f'{run_tool_one} config/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')
    elif nA < 5:
        f_run.write(f'{run_tool_half} config/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')
    else:
        f_run.write(f'{run_tool_full} config/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')

    f_eval.write("python -m tool.train --config config/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}.yaml \
training.resume='exp/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}/model/best_rob_acc.pth' training.evaluate_only=True \
evaluations.auto_attack_linf.eval_freq=1 evaluations.auto_attack_linf.eval_mode=True \
evaluations.auto_attack_linf.n_samples=9000 evaluations.auto_attack_linf.batch_size=200 \
evaluations.auto_attack_linf.save_path='exp/imagenet200/Linf_weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}/evaluations/auto_attack_l2' \
