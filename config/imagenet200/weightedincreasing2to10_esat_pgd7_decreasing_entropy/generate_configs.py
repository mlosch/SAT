...
print(f'#Instances: {ninstances}, #Bin-width: {bin_width}')
...
for i in range(1,N):
    ...
    if nA == 1:
        f_run.write(f'{run_tool_one} config/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')
    elif nA < 5:
        f_run.write(f'{run_tool_half} config/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')
    else:
        f_run.write(f'{run_tool_full} config/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{filename}.yaml\n')

    f_eval.write("python -m tool.train --config config/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}.yaml \
training.resume='exp/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}/model/best_rob_acc.pth' training.evaluate_only=True \
...
evaluations.auto_attack_l2.save_path='exp/imagenet200/weightedincreasing2to10_esat_pgd7_decreasing_entropy/{name}/evaluations/auto_attack_l2' \
