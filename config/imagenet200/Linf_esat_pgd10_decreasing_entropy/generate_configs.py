...
with open('./template.yaml', 'r') as f:
    template = f.read()
name_template = 'resnet50_A{nA}.PGDs1i10e8_B{nB}.none'
...
for i in range(1,N):
    ...
    cfg = template.format(A=str(A), B=str(B), nbins=N, ranking_filep=ranking_filep)
    ...
    if nA == 1:
        f_run.write(f'{run_tool_one} config/imagenet200/Linf_esat_pgd10_decreasing_entropy/{filename}.yaml\n')
    elif nA < 5:
        f_run.write(f'{run_tool_half} config/imagenet200/Linf_esat_pgd10_decreasing_entropy/{filename}.yaml\n')
    else:
        f_run.write(f'{run_tool_full} config/imagenet200/Linf_esat_pgd10_decreasing_entropy/{filename}.yaml\n')

    f_eval.write("sbatch tool/train_4h.sh config/imagenet200/Linf_esat_pgd10_decreasing_entropy/{name}.yaml \
training.resume='exp/imagenet200/Linf_esat_pgd10_decreasing_entropy/{name}/model/best_rob_acc.pth' training.evaluate_only=True \
...
evaluations.auto_attack_linf.save_path='exp/imagenet200/Linf_esat_pgd10_decreasing_entropy/{name}/evaluations/auto_attack_linf' \
